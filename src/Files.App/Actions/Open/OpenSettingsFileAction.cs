...
namespace Files.App.Actions
{
	...
	internal sealed partial class OpenSettingsFileAction : IAction
	{
		...
		public async Task ExecuteAsync(object? parameter = null)
		{
			try
			{
				var settingsJsonFile = await AppStorage.GetAppFileAsync(new Uri($"ms-appdata:///local/{Constants.LocalSettings.SettingsFolderName}/{Constants.LocalSettings.UserSettingsFileName}"));
